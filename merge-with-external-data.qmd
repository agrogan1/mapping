...
A common task in mapping is that we have a *shapefile* (@sec-shapefiles) of map data, but we want to merge in some *external data* from another source so that we can map that *external data*.

Often we want to use different colors to map that external data (@sec-symbology).

Here, I use a shapefile of countries of the world (@sec-shapefiles2), and merge that data with data from the World Bank World Development Indicators (@sec-WDI).
...
I am using the `wrld_simpl` dataset of country outlines that is available in several R packages (@sec-shapefiles2). I read this data into an object called `world`.
...
First I need to rename a variable in `WorldBankData`, so that the identifiers will match exactly.

```{r}
#| code-fold: true
#| code-summary: "Show the code"

WorldBankData$ISO3 <- WorldBankData$iso3c
...
newdata <- left_join(world, 
                     WorldBankData, by = "ISO3")
